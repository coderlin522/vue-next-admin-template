<template>
	<div class="layout-navbars-breadcrumb">
		<SvgIcon
			...
			:name="storesThemeConfig.themeConfig.isCollapse ? 'ele-Expand' : 'ele-Fold'"
			:size="16"
...
const onThemeConfigChange = () => {
	const value = !storesThemeConfig.themeConfig.isCollapse;
	// storesThemeConfig.themeConfig.isCollapse = !storesThemeConfig.themeConfig.isCollapse;
	storesThemeConfig.setThemeConfigIsCollapse(value);
	setLocalThemeConfig();
};
// 存储布局配置
...
const initRouteSplit = (path: string, route) => {
	if (!storesThemeConfig.themeConfig.isBreadcrumb) return false;
